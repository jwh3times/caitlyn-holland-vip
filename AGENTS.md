...
content only and does not apply Cloudflare's generated `out/_headers`; deployed-header validation
...
- **Post-deploy smoke — [.github/workflows/smoke.yml](.github/workflows/smoke.yml)** — runs on pushes to `main`, daily, and on manual dispatch. Push runs wait for the official Cloudflare Pages check on the exact pushed commit to succeed; daily/manual runs check the live site immediately. It checks out the repo and sets up Node, without installing dependencies, to run [scripts/smoke.mjs](scripts/smoke.mjs). The checker validates homepage and missing-page status/content, significant values in six security headers on both responses (including required script hashes and rejection of script `unsafe-inline`), and HTTP 200 responses from sitemap and robots. It fails on deployment failure or timeout; this is a post-deploy monitor, not a deployment gate.
...
**Security headers:** [public/\_headers](public/_headers) is the source template. `npm run build`
runs [scripts/generate-csp.mjs](scripts/generate-csp.mjs) after `next build` to hash inline scripts
in every exported HTML file, including 404 pages, and write the completed `out/_headers`.
The generator parses script text with JSDOM, preserves other headers, and fails the build if any
header-file line exceeds Cloudflare's 2,000-character limit. Deploy only the completed build:
the source template's `script-src 'self'` blocks inline scripts until hashes are added. The
generated policy permits the exported scripts, including the pre-paint theme script, by SHA-256
hash rather than script `unsafe-inline`; inline styles remain allowed for theme transitions.
