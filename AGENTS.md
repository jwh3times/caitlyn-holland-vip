...
- **Post-deploy smoke — [.github/workflows/smoke.yml](.github/workflows/smoke.yml)** — runs daily and on manual dispatch with no checkout or dependency install. It verifies the live homepage and content marker, every security header declared in [public/\_headers](public/_headers), and the deployed sitemap and robots file.
