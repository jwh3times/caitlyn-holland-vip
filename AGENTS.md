...
content only and does not apply Cloudflare's `out/_headers`; deployed-header validation
...
**Static-export constraints:** no API routes or `getServerSideProps`; images are `unoptimized` ([next.config.ts](next.config.ts)); all routes must be known at build time; no runtime environment variables. `next.config.ts` has no `headers()` block (ignored by static export anyway).

**Security headers:** [public/\_headers](public/_headers) is copied unchanged by `next build`.
`npm run build` then runs [scripts/validate-export-headers.mjs](scripts/validate-export-headers.mjs)
to check that `out/_headers` matches the source, the homepage and 404 exports exist, and the
security baseline and Cloudflare's 2,000-character line limit hold; it does not rewrite headers.
Script `unsafe-inline` is an accepted CSP limitation for Next.js bootstrap/hydration,
the next-themes pre-paint initializer, and Cloudflare JavaScript Detections, aligned with
holland.vip's accepted compatibility policy. The Analytics script host and beacon connections
are explicitly allowed; `base-uri 'self'`, `object-src 'none'`, `form-action 'self'`, and
framing restrictions remain. Inline styles remain allowed for theme transitions.
Static export does not force this exception: hashes are feasible but conflict with changing
edge scripts. Revisit stronger script policy for both sites together; generic CSP advice alone
does not justify a runtime server or per-request nonces.

Before changing the script CSP, read the [hash-policy evaluation](docs/research/static-export-hash-csp.md)
for measured browser compatibility and Cloudflare acceptance criteria. After building, run
`npm run benchmark:csp -- --policy=compatibility` to validate the accepted policy in Chromium,
Firefox, and WebKit, including error-boundary recovery; CI selects this mode with Chromium.
Add `--url=<url>` to check a hosted deployment of the same routes. The benchmark's default
remains the experimental hash policy, not the production policy; `--preview-output=<directory>`
prepares a separate hash-policy preview export. Retain the benchmark and research when changing
