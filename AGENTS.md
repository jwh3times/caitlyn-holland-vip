...
- **Post-deploy smoke — [.github/workflows/smoke.yml](.github/workflows/smoke.yml)** — runs on pushes to `main`, daily, and on manual dispatch. Push runs wait for the official Cloudflare Pages check on the exact pushed commit to succeed; daily/manual runs check the live site immediately. It checks out the repo and sets up Node, without installing dependencies, to run [scripts/smoke.mjs](scripts/smoke.mjs). The checker validates homepage and missing-page status/content, significant values in six security headers on both responses, and HTTP 200 responses from sitemap and robots. It fails on deployment failure or timeout; this is a post-deploy monitor, not a deployment gate.
